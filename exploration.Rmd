...
wbb_rosters26 <- read_csv("wbb_rosters_2025_26.csv") |> mutate(player_id = as.character(player_id))
...
non_us <- combined_rosters %>% 
  ...
  replace_na(list(`2024-25` = 0, `2023-24` = 0)) |> 
  mutate(pct_change = (`2024-25` - `2023-24`)/`2023-24` *100)
...
non_us_team <- combined_rosters %>% 
  ...
  replace_na(list(`2022-23` = 0, `2023-24` = 0, `2024-25` = 0)) |> 
  mutate(total = `2022-23` + `2023-24` + `2024-25`) |> 
  mutate(pct_change = (`2024-25` - `2023-24`)/`2023-24` *100)
```
...
```{r}
wbb_rosters24 |> filter(!is.na(previous_school_clean)) |> group_by(previous_school_clean) |> summarise(count = n()) |> arrange(desc(count))
```

```{r}
team_player_counts <- wbb_rosters24 |> group_by(ncaa_id, team) |> summarise(players = n())

team_transfer_counts <- wbb_rosters24 |> filter(!is.na(previous_school_clean)) |> group_by(ncaa_id, team) |> summarise(transfers = n())

teams_with_transfers_count <- team_player_counts |> left_join(team_transfer_counts) |> mutate(transfers = if_else(is.na(transfers), 0, transfers)) |> mutate(pct = transfers/players*100)
