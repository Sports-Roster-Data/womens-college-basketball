...
wbb_rosters <- wbb_rosters %>%
  mutate(pm.state = case_when(
    homestate %in% c("Hawai‘i", "O‘ahu", "Hawai'i") ~ "HI",
    homestate == 'Ken' ~ 'KY',
    homestate == 'Alta' ~ NA,
    homestate == 'Caif' ~ 'CA',
    homestate == 'Cal' ~ 'CA',
...
wbb_rosters <- wbb_rosters %>% 
  mutate(country = case_when(
    country %in% c('BC', 'QUEBEC', 'BRITISH COLUMBIA', 'ALBERTA', 'ONTARIO', 'NOVA SCOTIA', 'ONT', 'QUÉBEC', 'MANITOBA', 'QUE', 'ON', 'QC', 'ALBERTA (AB)', 'ONT CANADA', 'BC CANADA', 'BC,CANADA', 'ON CANADA', 'ALBERTA CANADA', 'SASKATCHEWAN', 'CN', 'AB', 'ALTA') ~ "CANADA", 
...
previous_schools <- previous_schools |> 
  mutate(previous_school_clean = case_when(
    ...
    previous_school == "Saint Peter's University" ~ "Saint Peter’s",
