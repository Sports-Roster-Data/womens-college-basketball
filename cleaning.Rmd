...
```{r, message=FALSE}
teams <- read_csv("https://raw.githubusercontent.com/dwillis/wbb-rosters/main/teams.csv") %>% select(-url, -twitter, -stats_name, -private)
...
```{r}
years_cleaned <- read_csv("https://raw.githubusercontent.com/dwillis/jour479x_fall_2022/main/data/years_cleaned.csv")
...
wbb_rosters23 <- wbb_rosters23 %>% 
  mutate(country = case_when(
    country %in% c('BC', 'QUEBEC', 'BRITISH COLUMBIA', 'ALBERTA', 'ONTARIO', 'NOVA SCOTIA', 'ONT', 'QUÉBEC', 'MANITOBA', 'QUE', 'ON', 'QC', 'ALBERTA (AB)', 'ONT CANADA', 'BC CANADA', 'BC,CANADA', 'ON CANADA') ~ "CANADA", 
    country %in% c('VICTORIA', 'SOUTH AUSTRALIA', 'WESTERN AUSTRALIA', 'TASMANIA', 'AUSTRAILA', 'AU') ~ 'AUSTRALIA',
...
wbb_rosters23 %>% group_by(country) %>% summarize(count = n()) %>% arrange(country)
...
```{r}
wbb_rosters23 <- subset(wbb_rosters23, select = -c(pm.id, pm.uid, pm.type, pm.address, has_roster, roster))
...
write_csv(wbb_rosters23, "wbb_rosters_2022_23.csv", quote="all", na="")
```
